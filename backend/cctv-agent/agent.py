...
- AGENT_TOKEN environment variable (get from web dashboard Settings page)
...
def load_config():
    """Fetch config from the backend using the AGENT_TOKEN."""
    import requests
    backend_url = os.environ.get("BACKEND_URL", "https://cctv-backup.onrender.com")
    agent_token = os.environ.get("AGENT_TOKEN")
    if not agent_token:
        logger.error(
            "AGENT_TOKEN not set. Get your pairing token from the web dashboard Settings page "
            "and set it as an environment variable: set AGENT_TOKEN=your_token_here"
        )
        sys.exit(1)
    try:
        resp = requests.get(
        ...
        if data.get("status") == "error":
            logger.error("Backend returned error: %s", data.get("message"))
            sys.exit(1)
        return data.get("config", {})
    except Exception as e:
        logger.error("Could not fetch config from backend: %s", e)
        sys.exit(1)
...
def run_sync(config, drive_service, folder_id, db_conn):
    ...
    try:
        recordings = find_recordings(cam, config.get("onvif_days_back", 3))
    except Exception as e:
        logger.error(
...
def main():
    ...
    interval_seconds = config.get("sync_interval", 60) * 60
